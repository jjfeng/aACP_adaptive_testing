...
class GraphicalParallelDP(GraphicalBonfDP):
    ...
    def __init__(self, base_threshold, alpha, success_weight, parallel_success_weight: float = 0.1, parallel_ratio: float = 0.9, model_corr: float = 0.99):
        self.base_threshold = base_threshold
        self.alpha = alpha
        self.success_weight = success_weight
    ...
    def set_num_queries(self, num_adapt_queries):
        ...
        node_dict = {i: [] for i in range(num_adapt_queries + 1)}
        node_dict[0] = [self.test_tree]
        self._create_tree(self.test_tree, num_adapt_queries, node_dict)
        ...
        for i in range(1, num_adapt_queries + 1):
            ...
            for node_same_level in node_dict[i - 1]:
                curr_par_node.weights[node_same_level] = (1 - self.parallel_success_weight)/len(node_dict[i - 1])
    ...
    def _get_test_eval(self, test_y, pred_y, alpha_level):
        """
        @return test perf where 1 means approve and 0 means not approved
        """
        test_y = test_y.flatten()
        ...
        upper_ci = np.mean(test_nlls) + t_stat_se * norm.ppf(1 - alpha_level)
        print("upper ci", alpha_level, np.mean(test_nlls), upper_ci)
        test_result = int(upper_ci < self.base_threshold)
        return test_result

    def get_test_eval(self, test_y, pred_y, predef_pred_y):
        print("HISTORY", self.test_tree.history, self.parallel_tree.alpha)
        predef_res = self._get_test_eval(test_y, predef_pred_y, self.parallel_tree.alpha)
        if predef_res == 1:
            print("PROPAGATE!!!!")
            # propagate to test tree
            # TODO: fix up propagation
            print(self.test_tree.alpha, "my old alpha")
            self.test_tree.earn(self.parallel_tree.alpha * self.parallel_tree.weights[self.test_tree])
            print(self.test_tree.alpha, "my new alpha")

            # propagate in parallel tree
            next_par_child = self.parallel_tree.par_child
            next_par_child.earn(self.parallel_tree.alpha * self.parallel_tree.weights[next_par_child])

        # Increment parallel tree
        self.parallel_tree = self.parallel_tree.par_child

        # TODO: calculate an actual correction factor to account for correlation.
        # This one is completely made up
        correction_factor = np.power(2, len(self.test_tree.history)) * self.model_corr
        test_result = self._get_test_eval(test_y, pred_y, self.test_tree.alpha * correction_factor)
